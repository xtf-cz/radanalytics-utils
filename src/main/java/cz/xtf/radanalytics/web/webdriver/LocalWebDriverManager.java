...
public class LocalWebDriverManager {
	...
	private static WebDriver setupRemoteChromeDriver(int podLocalPort) {
		...
		try {
			isRemoteHostReady(gridUrl, podLocalPort);
	...
	private static boolean isRemoteHostReady(String gridUrl, int podLocalPort) throws TimeoutException, InterruptedException {
		log.debug("Trying to get response code from {}", gridUrl);
		URL link = null;
		try {
			link = new URL(gridUrl);
		} catch (IOException e) {
			log.error(String.format("The following url provided is malformed: %s", gridUrl));
			log.error(e.getMessage());
		}
		URL finalLink = link;
